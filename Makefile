...
ROOTLIBS     := $(shell root-config --libs) -lSpectrum -lHistPainter
ROOTGLIBS    := $(shell root-config --glibs) -lSpectrum -lHistPainter
HASTHREAD    := $(shell root-config --has-thread)

CXX           = g++
CXXFLAGS      = -O3 -Wall -fPIC -Wunused-but-set-variable -std=gnu++11
LD            = g++
LDFLAGS       = -O3
SOFLAGS       = -shared
